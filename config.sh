...
install_module() {
    ...
    if [ $RELEASE == "8.1.0" ]; then    
        ...
        if $DS; then
            ...
            mkdir -p $MODPATH/system.priv-app/DataSpeedIndicator/oat 2>/dev/null
        ...
        if $SR; then
            ...
            mkdir -p $MODPATH/system/app/ScreenRecorder/oat 2>/dev/null            
        ...
        if $ST; then
            ...
            mkdir -p $MODPATH/system/priv-app/JunkCleaner/lib 2>/dev/null            
            ...
            mkdir -p $MODPATH/system/priv-app/VirusScan/lib 2>/dev/null
    ...
    if [ $RELEASE == "9" ]; then
        ...
        if $AI; then
            ...
            cp -af $AI_APK $MODPATH/system/product/CDA/items/AIFloatingTouch/AIFloatingTouch.apk
            ...
            cp -af $IS_APK $MODPATH/system/product/CDA/items/IntelligentSuggestion/IntelligentSuggestion.apk
            ui_print "  done!"
        else
            ui_print " "
            ui_print "AI floating touch not installed"
        ...
        if $SR; then
            ...
            cp -af $SR_APK $MODPATH/system/product/CDA/items/ScreenRecorder/ScreenRecorder.apk
